...
public class RSAPSS {
    ...
    public  RSAPSS()  {
        try{
            ...
            System.out.println("Error generating RSA key pair: " + e.getMessage());
        }

    }
    public RSAPSS(String privKey, String pubKey) throws Exception {
        KeyPairGenerator keyGen = KeyPairGenerator.getInstance("RSA");
        keyGen.initialize(2048);

        privateKey = RSAPSS.privateKeyFromString(privKey);
        publicKey = RSAPSS.publicKeyFromString(pubKey);
    }
